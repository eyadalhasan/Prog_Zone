...
class Course(models.Model):
    ...
    average_rank = models.FloatField(validators=[MinValueValidator(0), MaxValueValidator(5)], null=True, blank=True)
...
class StudentCourseRank(models.Model):
    ...
    class Meta:
        unique_together = ('student', 'course')  # Each student can rank a course only once

    def __str__(self):
        return f"{self.student.user.username} ranks {self.course.title} as {self.rank}"